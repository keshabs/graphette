...
{   
    ...
    if(G->sparse)
    {
	G->neighbor[i] = Realloc(G->neighbor[i], G->degree[i]*sizeof(G->neighbor[i][0]));
	G->neighbor[j] = Realloc(G->neighbor[j], G->degree[j]*sizeof(G->neighbor[j][0]));
	G->neighbor[i][G->degree[i]-1] = j;
	G->neighbor[j][G->degree[j]-1] = i;
    }
    else
    {
	SetAdd(G->A[i], j);
...
Boolean GraphsIsomorphic(GRAPH *G1, GRAPH *G2)
{
